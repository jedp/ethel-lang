...
static void cmp(type_t type, obj_t *a, obj_t *b, eval_result_t *result) {
    ...
    switch (type) {
        case AST_EQ:
            m = get_static_method(TYPEOF(a), METHOD_EQ);
            break;
        case AST_NE:
            m = get_static_method(TYPEOF(a), METHOD_NE);
            break;
        case AST_LT:
            m = get_static_method(TYPEOF(a), METHOD_LT);
            break;
        case AST_GT:
            m = get_static_method(TYPEOF(a), METHOD_GT);
            break;
        case AST_LE:
            m = get_static_method(TYPEOF(a), METHOD_LE);
            break;
        case AST_GE:
...
static boolean truthy(obj_t *obj) {
    switch (TYPEOF(obj)) {
        ...
        case TYPE_INT:
            return obj->intval != 0;
        case TYPE_FLOAT:
            return obj->floatval != 0;
        case TYPE_STRING:
            return obj->bytearray->size > 0;
        case TYPE_BYTE:
            return obj->byteval != 0x0;
        case TYPE_BOOLEAN:
            return obj->boolval;
        default:
    ...
    if (TYPEOF(args->root) != AST_EMPTY) {
        while (node != NULL) {
            ...
            switch (TYPEOF(result->obj)) {
                case TYPE_INT:
                    printf("%d ", result->obj->intval);
                    break;
                case TYPE_FLOAT:
                    printf("%f ", (double) result->obj->floatval);
                    break;
                case TYPE_BYTE:
                    printf("%c ", result->obj->byteval);
                    break;
                case TYPE_STRING:
                    printf("%s ", bytearray_to_c_str(result->obj->bytearray));
                    break;
                case TYPE_BOOLEAN:
                    printf("%s ", result->obj->boolval ? "true" : "false");
                    break;
                case TYPE_NIL:
                    printf("Nil ");
                    break;
                case TYPE_BYTEARRAY:
                    printf("%s", result->obj->bytearray->data);
                default:
                    printf("?? ");
                    break;
            }
